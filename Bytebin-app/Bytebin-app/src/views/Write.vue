    ...
    <el-container>
      ...
      <el-main>
        <div id="title">
          ...
          <el-button round style="float: right" type="primary" @click="SyntacticAnalysis">发布</el-button>
        </div>
...
import {getArticleDetail, postArticle, update, checkArticle} from "@/api/Article";
...
export default {
  ...
  methods: {
    ...
    SyntacticAnalysis(){
      ...
      checkArticle(this.article.content).then((res) => {
        if (res.data.request.flag == 1){
          console.log("无错误")
          console.log(res.data.request.multilineString)
          this.$confirm("通过检测，您的代码不存在语法错误",'',{
          ...
          let str = res.data.request.multilineString
          let trimmedStr = str.slice(1, -1);
          const blocks = trimmedStr.split('TempBlock');
          console.log(blocks)
          const newData = []
          const h = this.$createElement
          for (const i in blocks) {
            newData.push(h('p', null, blocks[i]))
          }
          this.$notify(
            // '提示',
          {
