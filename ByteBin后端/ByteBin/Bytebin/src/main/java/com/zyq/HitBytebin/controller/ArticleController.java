...
import com.zyq.HitBytebin.model.entity.Result;
...
import com.zyq.HitBytebin.serivce.SortService;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;
import com.zyq.HitBytebin.syn.MD;
...
public class ArticleController {
    ...
    @PostMapping("/check")
    public Result check(@RequestParam("content") String content) {
        Map<String, Object> data = new HashMap<>();
        try {
            data.put("request", MD.parse(content));
        } catch (IOException e) {
    ...
    @GetMapping("/pagenew/{page}")
    public Result getPageNew(@PathVariable("page") Integer page) {
        Map<String, Object> data = new HashMap<>();
        data.put("articles", articleService.getPageListNew(page));
        data.put("count", articleService.getActiveCount());
        return Result.ok(data);
    }
